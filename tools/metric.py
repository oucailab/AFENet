...
class Evaluator(object):
    ...
    def Precision(self):
        ...
        precision = tp / (tp + fp + self.eps)  # 防止除零
        return precision

    def Recall(self):
        tp, fp, tn, fn = self.get_tp_fp_tn_fn()
        recall = tp / (tp + fn + self.eps)  # 防止除零
        return recall
    ...
    def F1(self):
        ...
        F1 = (2.0 * Precision * Recall) / (Precision + Recall + self.eps)  # 防止除零
        return F1

    def Dice(self):
        tp, fp, tn, fn = self.get_tp_fp_tn_fn()
        Dice = 2 * tp / ((tp + fp + self.eps) + (tp + fn + self.eps))  # 防止除零
        return Dice
